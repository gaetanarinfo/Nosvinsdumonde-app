...
module.exports = {
    ...
    get: async (req, res) => {
        ...
        let sql = `SELECT V.nom AS nomBoisson, 
        V.id AS idBoisson,
        ...
        LEFT JOIN regions AS R ON R.id = V.region_id WHERE V.active = 1 ORDER BY V.created_at DESC LIMIT 12`;
    ...
    getOneVins: async (req, res) => {
        ...
        LEFT JOIN gout AS G ON G.id = V.gout_id
        LEFT JOIN pays AS P ON P.id = V.pays_id
        LEFT JOIN regions AS R ON R.id = V.region_id WHERE vin_jour = 1 AND V.active = 1 ORDER BY V.id DESC LIMIT 1`;
        db.query(sql, (error, data, fields) => {
            if (error) throw error;
            res.json({
                status: 200,
    ...
    getId: (req, res) => {
        ...
        LEFT JOIN degustations AS D ON D.vin_id = V.id AND D.lang = "${language}"
        LEFT JOIN vins_saviez_vous AS VSV ON VSV.vin_id = V.id AND VSV.lang = "${language}"
        LEFT JOIN definitions AS DF ON DF.vin_id = V.id AND DF.lang = "${language}" WHERE V.boisson_id = 1 AND V.active = 1 AND V.id = "${req.params.id}"`;
        ...
        if (req.params.id != undefined) {
            ...
            LEFT JOIN gout AS G ON G.id = V.gout_id
            LEFT JOIN pays AS P ON P.id = V.pays_id
            LEFT JOIN regions AS R ON R.id = V.region_id WHERE V.appellation_id = "${data[0].apellationIdBoisson}" AND V.id != "${req.params.id}" AND V.boisson_id = 1 AND V.active = 1 ORDER BY V.created_at DESC LIMIT 3`;
