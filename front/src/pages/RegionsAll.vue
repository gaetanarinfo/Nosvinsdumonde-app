<template>
  <div class="row items-start" style="padding: 0 5vw; flex-direction: column">
    <div class="text-h4 text-white bloc_vin_title" style="
        ...
        margin: 24px 0;
      ">
      <q-item class="title">{{ listRegionsTitle.nom_fr_fr }}</q-item>
    </div>
    ...
    <div class="q-pa-none q-gutter-sm">
      <q-breadcrumbs class="text-brown">
        ...
        <q-breadcrumbs-el :label="listRegionsTitle.nom_fr_fr" style="color: white" />
      </q-breadcrumbs>
    </div>

    <div class="q-mb-lg q-mt-lg" v-show="showSimulatedReturnData"
      style="justify-content: center; display: flex; width: 100%">
      <q-pagination v-model="current" :max="max" :max-pages="7" @click="chargeRegions()" direction-links outline
    ...
    <div class="items-start q-gutter-md justify-center" style="margin-bottom: 0; padding: 0 0; width: 100%"
      ...
      <q-card class="card_vin q-mb-lg" flat v-for="champagnes_vins in listRegionsAll" :key="champagnes_vins.idBoisson">
...
export default {
  ...
  methods: {
    ...
    ...mapActions('regions', ['getRegionsAll']),
    ...mapActions('regions', ['getRegionsTitle']),
    ...mapActions('regions', ['getRegionsCount']),
    chargeRegions() {
      this.getRegionsAll({
        page: this.current,
    ...
    countVins() {
      if (counter <= 1) {
        setTimeout(() => {
          if (this.listRegionsCount <= 19) this.max = 1;
          else this.max = parseInt(this.listRegionsCount);
          counter++;
        }, 300);
      }
    },
  ...
  mounted() {
    ...
    this.getRegionsAll({ page: this.current, id: this.$route.params.id });
    this.getRegionsTitle(this.$route.params.id);
    this.getRegionsCount(this.$route.params.id);
    this.countVins();
  ...
  computed: {
    ...mapState('regions', ['listRegionsAll']),
    ...mapState('regions', ['listRegionsTitle']),
    ...mapState('regions', ['listRegionsCount']),
  },
};
</script>
