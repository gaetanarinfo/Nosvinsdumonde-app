<template>
  <div class="row items-start" style="padding: 0 5vw; flex-direction: column">
    ...
    <div class="q-mb-lg q-mt-lg" v-show="showSimulatedReturnData" style="
      ...
      <div v-if="listBonPlansAll.length !== 0" style="justify-content: center; display: flex; width: 100%"
        v-show="showSimulatedReturnData">

        <q-pagination v-model="page" :input="true" input-class="text-info" :min="currentPage"
          :max="Math.ceil(listBonPlansAll.length / totalPages)" direction-links outline color="orange"
    ...
    <div class="items-start q-gutter-md justify-center" style="margin-bottom: 0; padding: 0 0; width: 100%"
      ...
      <q-card id="my-table" class="card_vin q-mb-lg" flat v-for="(bon_plans, index) in getData" :key="index">
        ...
        <q-card-section>
          <q-item clickable :to="'/' + bon_plans.typeBoisson + '/' + bon_plans.idBoisson"
            class="text-h6 q-mt-sm q-mb-xs text-bold text-black" style="padding: 0 0">
            {{ decode(bon_plans.nomBoisson) }} {{ bon_plans.millesimeBoisson }}
          </q-item>
          <div class="q-mt-lg">
...
export default {
  ...
  components: {},
  methods: {
    ...mapActions('bon_plans', ['getBonPlansAll']),
    ...mapActions('bon_plans', ['getPays']),
    ...mapActions('bon_plans', ['getApellation']),
    ...mapActions('bon_plans', ['getRegion']),
  ...
  computed: {
    ...mapState('bon_plans', ['listBonPlansAll']),
    ...mapState('bon_plans', ['listPays']),
    ...mapState('bon_plans', ['listApellation']),
    ...mapState('bon_plans', ['listRegion']),
    getData() {
      return this.listBonPlansAll.slice((this.page - 1) * this.totalPages, (this.page - 1) * this.totalPages + this.totalPages)
    },
  },
  mounted() {
    this.getBonPlansAll(this.form);
