...
const mutations = {
  ...
  setListVinsAll(state, value) {
    state.listVinsAll = value
  },
...
const actions = {
  ...
  getVinsAll({
    ...
    axios
      .post('/vinsAll/', vins)
      .then(res => {
        if (res.data.listVinsAll !== 0) {
          commit('setListVinsAll', res.data.listVinsAll)
        } else {
          commit('setListVinsAll', [])
        }
      })
  },
  getPays({
...
const getters = {
  ...
  setListVinsAll: state => {
    return state.listVinsAll
  },
  setListPays: state => {
